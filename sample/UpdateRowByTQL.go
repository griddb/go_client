...
func main() {
    ...
    containerName := "SampleGo_UpdateRowByTQL"
    rowCount := 5
    ...
    for i := 0; i < rowCount; i++ {
        id := i
        rowList = append(rowList, []interface{}{id, nameList[i], numberList[i]})
        fmt.Println("Sample data generation:  row=(", id, ", ", nameList[i], ", ", numberList[i], ")")
    }
    ...
    qr, err3 := col1.Query("SELECT * WHERE id = 4")
    if err3 != nil {
    ...
    //(4)Get the result
    for rs.HasNext() {
        _ , err4 := rs.NextRow() //Avoid invalid results returned
        if (err4 != nil) {
            fmt.Println("GetNextRow err:", err4)
            panic("err GetNextRow")
        }

    //Update a row
        err5 := rs.Update([]interface{}{2, "keyboard", 325})
        if err5 != nil {
